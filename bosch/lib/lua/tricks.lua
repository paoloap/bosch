...
-- @release 0.8.1
-- @module bosch.lib.tricks
---------------------------------------------------------------------------

local tricks = { _NAME = "bosch.lib.tricks" }
...
function tricks.table_tostring(tbl)
   ...
   for k, v in pairs(tbl) do
      if not done[k] then
         table.insert(result, tricks.key_to_str(k) .. "=" .. tricks.val_to_str(v))
      end
   end
...
function tricks.val_to_str(v)
   if "string" == type(v) then
      ...
      return "table" == type(v) and tricks.table_tostring(v) or tostring(v)
   end
end
